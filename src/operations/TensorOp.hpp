 ...
 */
extern "C" void LaunchPowerKernel(CustomOpDataType dataType, void *inputBuffer,
                                  int linearLength, void *power,
                                  void *outputBuffer);
  ...
  private:
    void ForwardAdd() {
        ...
        LOG.DEBUG() << "Launch PW_ADD FWD"
                    ...
                    << m_inputB->PrintShape()
                    << "\n  out: " << m_output->GetName() << m_output->PrintShape()
                    << "\n  blend factors " << blendFactors[0] << ", "
                    << blendFactors[1] << ", " << blendFactors[2]
