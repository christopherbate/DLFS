...
int DLFS::writeBMPi(const char *filename, const unsigned char *d_RGB, int pitch,
    ...
    checkCudaErrors(cudaMemcpy2D(chanRGB, (size_t)width * 3, d_RGB,
                                 (size_t)pitch, width * 3, height,
         ...
         y--) // BMP image format is written from bottom to top...
    {
        ...
        if (extrabytes) // See above - BMP lines must be of lengths divisible
                        // by 4.
        {
...
void DLFS::WriteImageTensorBMP(const char *filename,
