 ...
 */
template <typename T>
ConvLayer<T>::ConvLayer(cudnnHandle_t &handle, int filterSize, int stride) : Layer<T>(handle)
{
    m_filterTd = NULL;
    m_convDesc = NULL;
    m_convAlg = CUDNN_CONVOLUTION_FWD_ALGO_IMPLICIT_PRECOMP_GEMM;
...
template <typename T>
void ConvLayer<T>::AllocateOutputBuffers()
{
    // if (this->m_outputBuffer != NULL || m_workspaceBuffer != NULL)
    // {
    //     throw std::runtime_error("Attempted to allocate output buffer in ConvLayer without releasing previous output/workspace buffers.");
    // }
    // if (this->m_filterBuffer == NULL)
    // {
    //     throw std::runtime_error("Attempated to allocate output buffer in ConvLayer without first creating filter weights.");
    // }
...
template <typename T>
void ConvLayer<T>::AllocateWeightBuffers()
{
    // if (m_filterBuffer != NULL)
    // {
    //     throw std::runtime_error("Attempted to allocate filter buffer in ConvLayer without releasing previous filter buffer.");
    // }
...
void ConvLayer<T>::SetFilerDim(FilterDims &dims)
{
    ...
    // if (m_filterTd == NULL)
    // {
    //     throw std::runtime_error("Attempted to set filter dimensions in ConvLayer, but no filter descriptor was created.");
    // }

    // m_filterDims = dims;
    // checkCudaErrors(cudnnSetFilter4dDescriptor(m_filterTd, CUDNN_DATA_FLOAT,
    //                                            CUDNN_TENSOR_NCHW,
    //                                            /*output feature maps*/ dims.outputFeatures,
    //                                            /*input feature maps*/ dims.inputFeatures,
...
void ConvLayer<T>::FindBestAlgorithm()
{
    ...
    //     cout << "  Mem:" << perfResults[i].memory << ", "
    //          << " Time: " << perfResults[i].time << endl;
    // }
...
{
    if(this->m_outputTd == NULL){
        throw std::runtime_error("Attempted to call GetAlgWorkspaceNeeded, but output buffer descriptor is not created.");
    }
