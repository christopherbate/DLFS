 ...
 * The basic idea is simply add several tests, and let the singleton take care
 * of keeping track of managing the tests. Use the assertion methods within
 * QuickTest class within testing code.
 */
#ifndef CTB_TEST
#define CTB_TEST
#include <iomanip>
...
#define QTNotEqual(val1, val2)                                                 \
    QuickTest::NotEqual(val1, val2, __FILE__, __LINE__)
#define QTAlmostEqual(val1, val2, eps)                                         \
    QuickTest::AlmostEqual(val1, val2, eps, __FILE__, __LINE__)
...
class QuickTestError : public std::exception {
  public:
    QuickTestError(const std::string &errorMsg) : m_errorMsg(errorMsg) {}
    const char *what() { return m_errorMsg.c_str(); }
...
class QuickTest {
  public:
    template <typename T1, typename T2> static void Equal(T1 a, T2 b) {
    ...
    static void NotEqual(void *a, void *b) {
        if (a == b) {
            std::string msg = "Equal: " + std::to_string(uint64_t(a)) +
                              " != " + std::to_string(uint64_t(b));
    ...
    static void AlmostEqual(float a, float b, float eps, const char *file,
                            const int line) {
        if (std::abs(a - b) > eps) {
            std::stringstream ss;
            ss << "Test Failure at " << file << ":" << line << ": " << a
               << " != " << b << " "
               << " (AlmostEqual)" << (a - b) << " > " << eps;
            throw QuickTestError(ss.str());
        }
...
class Test {
  public:
    ...
    void Run(bool tty) {
        ...
        try {
            func();
            std::cout << (tty ? GRN : "") << "Test Passed" << (tty ? RST : "")
                      << std::endl;
            fail = false;
        } catch (QuickTestError &e) {
            std::cout << (tty ? RED : "") << "Test Failed \n";
            std::cout << e.what() << (tty ? RST : "") << std::endl;
            fail = true;
        } catch (std::exception &e) {
            std::cout << (tty ? RED : "") << "Test Failed, Fatal Exception \n";
            std::cout << e.what() << (tty ? RST : "") << std::endl;
            fail = true;
        }
    }
...
class TestCase {
  public:
    TestCase(std::string name) : m_name(name), failCount(0) {}
    ...
    int GetNumFailures() { return failCount; }
    void PrintFailures(bool tty) {
        for (auto it = tests.begin(); it != tests.end(); it++) {
            std::cout << (tty ? RED : "");
            if ((*it)->fail) {
