...
#include "lib/operations/Softmax.hpp"
...
void TestSoftmax() {
    ...
    TestRunner::GetRunner()->AddTest(
        "Sigmoid Cross Entropy Op",
        "Sigmoid Cross Entropy Forward/ Backward (Uneduced)", []() {
            ADContext.Reset();
            ...
            TensorPtr<float> logits =
                CreateTensor<float>(shape, "logits", 1.0, true);
    ...
    TestRunner::GetRunner()->AddTest(
        "Sigmoid Cross Entropy Op",
        "Sigmoid Cross Entropy Forward/ Backward (Reduced)", []() {
            ADContext.Reset();
